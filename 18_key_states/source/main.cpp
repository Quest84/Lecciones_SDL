...
bool init() {
    ...
    if( SDL_Init( SDL_INIT_VIDEO ) < 0 ) {
        ...
        // Crea la ventana
        gWindow = SDL_CreateWindow( "SDL Tutorial 18 - Key States", 
...
bool loadMedia() {
    ...
    // Carga la textura presionada
    if( !gPressTexture.loadFromFile( "romfs/press.png" ) ) {
        printf( "Falló la carga de la textura!\n" );
        success = false;
    }
    if( !gUpTexture.loadFromFile( "romfs/up.png" ) ) {
        printf( "Falló la carga de la textura!\n" );
        success = false;
    }
    if( !gDownTexture.loadFromFile( "romfs/down.png" ) ) {
        printf( "Falló la carga de la textura!\n" );
        success = false;
    }
    if( !gLeftTexture.loadFromFile( "romfs/left.png" ) ) {
        printf( "Falló la carga de la textura!\n" );
        success = false;
    }
    if( !gRightTexture.loadFromFile( "romfs/right.png" ) ) {
        printf( "Falló la carga de la textura!\n" );
        success = false;
    }
...
int main( int argc, char* argv[] ) {
    ...
    // Textura renderizada actualmente
    ...
    while( !quit ) {
        ...
        // Establece la textura basada en el estado actual de las teclas
