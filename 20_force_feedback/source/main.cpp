...
bool init() {
    ...
    if( SDL_Init( SDL_INIT_VIDEO | SDL_INIT_JOYSTICK | SDL_INIT_HAPTIC ) < 0 ) {
        ...
        if( SDL_NumJoysticks() < 1 ) {
            ...
            if( gGameController == NULL ) {
                ...
                if( !gControllerHaptic == NULL) {
                    printf( "Warning: El control no soporta vibración! SDL Error: %s\n",
                            SDL_GetError() );
                } else {
                    // Inicializa el rumble
                    if( SDL_HapticRumbleInit( gControllerHaptic ) < 0 ) {
                        printf( "Warning: No se pudo inicializar el rumble! SDL Error: %s\n",
                                SDL_GetError() );
                    }
                }
            }
        ...
        // Crea la ventana
        gWindow = SDL_CreateWindow( "SDL Tutorial 20 - Force Feedback", 
...
void close() {
    ...
    // Cierra el control y la vibración
    ...
    while( !quit ) {
        while( SDL_PollEvent( &e ) != 0 ) {
            switch( e.type ) {
                ...
                // Activa la vibración al presionar un botón
                case SDL_JOYBUTTONDOWN:
                      if( SDL_HapticRumblePlay( gControllerHaptic, 0.75, 500 ) != 0 ) {
                          printf( "Warning: No se pudo activar la vibración! %s\n",
                                  SDL_GetError() );
                      }
            }
        }
